...
__plugin_meta__ = PluginMetadata(
    name="HowToCook 菜谱",
    description="HowToCook 周计划、购物清单、忌口筛选、智能推荐与完整菜谱客户端",
        ...
        "menu_data": [
            {
                ...
                "trigger_method": (
                    ...
                    "- **命令**：`<命令前缀>做饭 配餐 [--荤 1 --素 1 --汤 1 --忌口 辣]`\n"
                    "- **命令**：`<命令前缀>做饭 周计划 [天数] [--最高难度 3 --忌口 海鲜]`\n"
                ...
                "detail_des": (
                    "随机、配餐与 1–14 天周计划支持固定种子和忌口标签；配餐/周计划卡片"
                    "可直接回复“购物清单 4”，将全部菜谱按 4 人份归一汇总。独立购物清单"
                    "命令同时接受稳定 ID 与完整菜名。食材找菜仍支持覆盖率与严格齐全模式。"
                    "所有候选共用序号选择，一个候选会直接打开完整详情。饮食标签来自原料"
                    "启发式识别，不替代医学过敏判断。\n\n## 示例\n\n"
                    "- `<命令前缀>做饭 周计划 7 --忌口 海鲜,花生`\n"
                    "- 周计划卡片后回复 `购物清单 4`\n"
                ...
                "pmn_triggers": [
                    ...
                    {"type": "command", "label": "命令", "value": "<命令前缀>做饭 周计划 [天数]"},
                    {
                        "type": "command",
                        "label": "命令",
            ...
            },
            {
                ...
                "detail_des": (
                    ...
                    "工具、步骤、备注、正文与成品图。原料接口支持按 1–100 人份换算数值用量"
                    "并标记无法自动换算的原文；JSON-LD 提供 schema.org Recipe 数据。"
                    "日常使用无需复制 ID。"
                ),
                "pmn_hidden": False,
                "pmn_triggers": [
